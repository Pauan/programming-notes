...
# TODO maybe make this into a primitive, and have an argument for the buffer size ?
# TODO even if the consumer consumes as fast as possible, it still drops every other value
(FUNCTION
  # TODO this name isn't quite correct
  (dropping :: (-> (Stream a) (Stream a)))
  (dropping in)
    (make-stream -> out
      (with-stream in -> in
        (each-output! in -> value
